...
export function recipientSmsPayload(ctx: OrderNotifyContext): string | null {
  ...
  if (ctx.hidePrice) {
    return `Yanney Trendss: A package is on its way to you 💖 Delivery update for order ${ctx.orderRef}.`;
  }
  return `Yanney Trendss: Your order ${ctx.orderRef} is on the way.`;
}
