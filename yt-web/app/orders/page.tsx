...
export const metadata = {
  title: "My Orders | Yanney Trendss",
};
...
function OrderHistoryItem({ order }: { order: ReceiptOrder }) {
  ...
  return (
    <Link
      ...
      className="block rounded-lg border border-brand-border bg-brand-surface p-4 transition hover:shadow-md"
    >
      <div className="flex items-start justify-between">
        ...
        <span className="rounded-full bg-brand-pink-muted px-3 py-1 font-jost text-xs font-medium text-brand-pink">
          {order.status}
        </span>
      </div>
      <p className="mt-2 font-jost text-lg font-semibold text-brand-text">
...
export default async function OrderHistoryPage() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-brand-surface py-24 text-center">
        <h1 className="font-playfair text-2xl text-brand-text">My Orders</h1>
        ...
        <p className="mt-2 font-jost text-sm text-brand-muted">
          Your account is created automatically when you verify your phone number at checkout.
        </p>
        <Link
          href="/shop"
          className="mt-8 inline-block rounded-lg bg-brand-pink px-6 py-3 font-jost text-sm font-semibold text-white"
        >
          Continue Shopping
        </Link>
      </div>
  ...
  if (error) console.error("Error fetching order history:", error);
  ...
  return (
    <div className="min-h-screen bg-brand-surface py-24">
      <div className="mx-auto max-w-4xl px-4">
        ...
        <p className="mt-2 font-jost text-sm text-brand-muted">
          Your past and current orders with Yanney Trendss.
        </p>
        ...
        {orders && orders.length > 0 ? (
          ...
          <div className="mt-12 text-center">
            ...
            <Link
              href="/shop"
              className="mt-6 inline-block rounded-lg bg-brand-pink px-6 py-3 font-jost text-sm font-semibold text-white"
            >
              Start Shopping
            </Link>
          </div>
